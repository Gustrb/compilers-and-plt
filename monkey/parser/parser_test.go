...
import (
	...
	"github.com/gustrb/compilers-and-plt/monkey/ast"
...
func TestOperatorPrecedenceParsing(t *testing.T) {
	tests := []struct {
		...
		{input: "5 < 4 != 3 > 4", expected: "((5 < 4) != (3 > 4))"},
		{input: "3 + 4 * 5 == 10", expected: "((3 + (4 * 5)) == 10)"},
		...
		{input: "a * [1, 2, 3][b * c] * d", expected: "((a * ([1, 2, 3][(b * c)])) * d)"},
...
func TestIfExpression(t *testing.T) {
	...
	if !ok {
		t.Fatalf("consequence[0] is not ast.ExpressionStatement. got=%T", exp.Consequence.Statements[0])
	}
...
func TestIfElseExpression(t *testing.T) {
	...
	if exp.Alternative == nil {
		t.Fatalf("exp.Alternative is nil. got=%+v", exp.Alternative)
	}
...
func TestFunctionLiteralParsing(t *testing.T) {
	...
	if len(funcLit.Parameters) != 2 {
		t.Fatalf("function literal parameters wrong. want 2, got=%d", len(funcLit.Parameters))
	}

	testLiteralExpression(t, funcLit.Parameters[0], "x")
	testLiteralExpression(t, funcLit.Parameters[1], "y")

	if len(funcLit.Body.Statements) != 1 {
		t.Fatalf("function literal body is not 1 statements. got=%d", len(funcLit.Body.Statements))
	}

	bodyStmt, ok := funcLit.Body.Statements[0].(*ast.ExpressionStatement)
	if !ok {
		t.Fatalf("bodyStmt is not ast.ExpressionStatement. got=%T", funcLit.Body.Statements[0])
	}
...
func TestFunctionParameterParsing(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(funcLit.Parameters) != len(tt.expectedParams) {
			t.Fatalf("function literal parameters wrong. want %d, got=%d", len(tt.expectedParams), len(funcLit.Parameters))
		}

		for i, ident := range funcLit.Parameters {
...
func TestCallExpressionParsing(t *testing.T) {
	...
	if len(call.Arguments) != 3 {
		t.Fatalf("call.Arguments does not contain 3 arguments. got=%d", len(call.Arguments))
	}
...
func TestArrayLiteralParsing(t *testing.T) {
	...
	if len(literal.Elements) != 3 {
		t.Fatalf("array literal does not contain 3 elements. got=%d", len(literal.Elements))
	}

	testLiteralExpression(t, literal.Elements[0], 1)
...
func testLiteralExpression(t *testing.T, exp ast.Expression, expected interface{}) bool {
	...
	t.Errorf("type %T not handled", expected)
	return false
}
